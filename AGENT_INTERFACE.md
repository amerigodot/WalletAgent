...
Returns the current API status and a list of available endpoints.
...
Returns a list of recent transaction and lifecycle events.
...
- **Storage:** Military-grade encryption (Argon2id + XSalsa20-Poly1305) for the wallet secret.
...
Implemented as an **API-accessible Polled Log**. Agents and humans can query the internal state via the `/agent/audit` endpoint.
...
| `GET`  | `AUDIT_LOGS`    | `limit: 50`                               | `GET /agent/audit`     |
